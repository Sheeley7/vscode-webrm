...
export class CrmWebAPI {
    ...
    static async getWebResources(connection: Connection, solution: Solution) {
        ...
        let solutionItems = await this.getRecords(connection, "/api/data/v"+ apiVersion +"/solutioncomponents?$select=objectid&$filter=_solutionid_value eq "+solution.getSolutionId()+" and  componenttype eq 61");
        ...
        var wrSelect = "/api/data/v"+ apiVersion +"/webresourceset?$select=content,contentjson,description,displayname,iscustomizable,ismanaged,name,webresourceid,webresourceidunique&$filter=";
        var requests = [];
        for(var i = 0; i < chunked_arr.length; i++) {
            ...
            for(var j = 0; j < chunked_arr[i].length; j++) {
                if(j === 0) {
                    ...
                    filter += " or webresourceid eq " + chunked_arr[i][j].objectid;
                }
            }
        ...
        try {
            ...
            for(i = 0; i < webResources.length; i++) {
                ...
                for(j = 0; j < currentBlock.length; j++) {
                    ...
                    webResourceResults.push(new WebResource(currWebResource.name, currWebResource.webresourceid, fileName, fullFilePath, currWebResource.content, "file"));
    ...
    static async publishWebResource(connection: Connection, webResourceId: string, base64Content: string) {
        try {
            let record: any = {};
            record.content = base64Content;
            let updated = await this.updateRecord(connection, record, "webresourceset", webResourceId);
            if(updated) {
                let published = await this.publishXML(connection, webResourceId);
            }
        } catch(err) {
            throw new Error(err);
        }
    ...
    private static async updateRecord(connection: Connection, record: any, entityName: string, recordId: string) {
        return new Promise<boolean>(function(resolve, reject){
            ...
            var options = {
                url: connection.getConnectionURL() + "/api/data/v"+ apiVersion +"/" + entityName + "(" + recordId + ")",
                method: "PATCH",
                json: record,
                headers: headers
            };
    ...
    private static async publishXML(connection: Connection, webResourceId: string) {
        return new Promise<boolean>(function(resolve, reject){
            let apiVersion = getConfig().get('dynamicsAPIVersion');
            let parameters = {"ParameterXml": "<importexportxml><webresources><webresource>{"+webResourceId+"}</webresource></webresources></importexportxml>"};
            ...
            var options = {
                url: connection.getConnectionURL() + "/api/data/v"+ apiVersion +"/PublishXml",
                method: "POST",
                json: parameters,
                headers: headers
            };
