...
export const AuthServer = async (connectionURL: string) => {
  if (getConfig().get("useLocalAuth") === true) {
    ...
    await vscode.commands.executeCommand(
      "vscode.open",
      vscode.Uri.parse(
        getConfig().get("authWebServiceURL") + "/login?crm_url=" + connectionURL
      )
    );
    ...
    try {
      ...
      app.get("/result", (request: any, reponse: any) => {
        ...
        reponse.redirect(
          `${getConfig().get(
            "authWebServiceURL"
          )}/?message=${encodeURIComponent("You can now close this tab")}`
        );
        request.destroy();
      });
      ...
      vscode.commands.executeCommand(
        "vscode.open",
        vscode.Uri.parse(
          getConfig().get("authWebServiceURL") +
            "/login?crm_url=" +
            connectionURL
        )
      );
...
const LocalAuthServer = async (connectionURL: string) => {
  ...
  const createServer = new Promise<AuthServerPromise>((resolve, reject) => {
    ...
    try {
      ...
      vscode.commands.executeCommand(
        "vscode.open",
        vscode.Uri.parse(redirect_uri + "/auth")
      );
    } catch (err) {
      reject(err);
    }
