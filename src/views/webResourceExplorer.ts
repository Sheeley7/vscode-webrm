...
export class WebResourceExplorer implements vscode.TreeDataProvider<WebResource> {
    ...
    getTreeItem(element: WebResource): vscode.TreeItem | Thenable<vscode.TreeItem> {
        return element;
    }
                ...
                for(var j = 0; j < webResourcePathParts.length; j++) {
                    ...
                    if(j !== webResourcePathParts.length - 1) {
                        if(typeof(prevPart[currFilePart]) === "undefined") {
                            ...
                            //Create folder node
                            let newWR = new WebResource(currFilePart, "", "", "", "", "folder");
...
export class WebResource extends vscode.TreeItem {
    ...
    constructor(webResourceName:string, webResourceId:string,  fileName: string, fullPath:string,webResourceContent: string, type: string) {
        if(type !== "folder") {
